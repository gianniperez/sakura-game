...
El objetivo de este proyecto es desarrollar un videojuego a partir de un `entorno.jar` en Java. El videojuego es sobre Sakura, quien tiene una florería que suele ser atacada por ninjas enemigos. Esto complica su trabajo, el cual consiste en entregar los pedidos de flores evitando y combatiendo a los ninjas enemigos para no perder la vida.
